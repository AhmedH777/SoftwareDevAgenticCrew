...
from crew import ArchitectCrew
...
def run(inputs=None):
    ...
    """  
    try:
        ArchitectCrew(llm=llm).crew().kickoff(inputs=inputs)
...
def train(inputs = None, n_iterations = 1, filename = "training.pkl"):
    ...
    """
    try:
        ArchitectCrew(llm=llm).crew().train(n_iterations=n_iterations, filename=filename, inputs=inputs)
...
def replay(task_id = None):
    ...
    """
    try:
        ArchitectCrew(llm=llm).crew().replay(task_id=task_id)
...
def test(inputs = None, n_iterations = 1, openai_model_name = "gpt-4o"):
    ...
    try:
        ...
        ArchitectCrew(llm=llm).crew().test(n_iterations=n_iterations, openai_model_name=openai_model_name, inputs=inputs)
